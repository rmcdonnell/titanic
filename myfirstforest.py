...
# Embarked from 'C', 'Q', 'S'
# Note this is not ideal: in translating categories to numbers, Port "2" is
# not 2 times greater than Port "1", etc.

# All missing Embarked -> just make them embark from most common place
if len(train_df.Embarked[ train_df.Embarked.isnull() ]) > 0:
    train_Embarked = train_df.Embarked.dropna().mode().values
    train_df.loc[ train_df.Embarked.isnull(), 'Embarked' ] = train_Embarked
...
# TEST DATA
test_df = pd.read_csv('data/test.csv', header=0)          # Load the test file into
                                                     # a dataframe
...
if len(test_df.Embarked[ test_df.Embarked.isnull() ]) > 0:
    test_Embarked = test_df.Embarked.dropna().mode().values
    test_df.Embarked[ test_df.Embarked.isnull() ] = test_Embarked
...
if len(test_df.Fare[ test_df.Fare.isnull() ]) > 0:
    ...
    for f in range(0,3):                                         # loop 0 to 2
        temp = test_df[ test_df.Pclass == f+1 ]['Fare'].dropna().median()
        median_fare[f] = temp
    for f in range(0,3):                                         # loop 0 to 2
